...
abstract class DataObject implements FhirCompatible
{
	...
	static public function fromFhir($fhirObject)
	{
		...
		$schema = \Yii::app()->fhirMarshal->getSchema($fhirType);
		...
		$values = static::getFhirTemplate()->match($fhirObject, $warnings);
		if (is_null($values)) {
			throw new InvalidStructure("Failed to match object of type '{$fhirType}': " . implode("; ", $warnings));
		}
