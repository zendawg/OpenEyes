...
{
	protected $tag = 'http://openeyes.org.uk/fhir/tag/resource/Practitioner/Gp';
	...
	{
		$source = file_get_contents(__DIR__ . '/files/Gp.xml');
		$this->post('Practitioner', $source, array('Category' => "{$this->tag}; scheme=\"http://hl7.org/fhir/tag\""));
	...
	{
		$this->get('Practitioner?_id=gp-1&_tag=' . urlencode($this->tag));
		$this->assertXPathEquals('feed', 'local-name()');
		$this->assertXPathEquals($this->client->getBaseUrl(), 'string(./atom:link[@rel="base"]/@href)');
		$this->assertXPathEquals($this->client->getBaseUrl() . '/Practitioner?_id=gp-1&_tag=' . urlencode($this->tag), 'string(./atom:link[@rel="self"]/@href)');
	...
	{
		$this->get('Practitioner?_id=gp-666666&_tag=' . urlencode($this->tag));
		$this->assertXPathEquals('feed', 'local-name()');
		$this->assertXPathEquals($this->client->getBaseUrl(), 'string(./atom:link[@rel="base"]/@href)');
		$this->assertXPathEquals($this->client->getBaseUrl() . '/Practitioner?_id=gp-666666&_tag=' . urlencode($this->tag), 'string(./atom:link[@rel="self"]/@href)');
		$this->assertXPathCount(0, './atom:entry');
	}

	public function testSearchByGnc()
	{
		$this->get('Practitioner?identifier=QRST&_tag=' . urlencode($this->tag));
		$this->assertXPathEquals('feed', 'local-name()');
		$this->assertXPathEquals($this->client->getBaseUrl(), 'string(./atom:link[@rel="base"]/@href)');
		$this->assertXPathEquals($this->client->getBaseUrl() . '/Practitioner?identifier=QRST&_tag=' . urlencode($this->tag), 'string(./atom:link[@rel="self"]/@href)');
		$this->assertXPathCount(1, './atom:entry');
