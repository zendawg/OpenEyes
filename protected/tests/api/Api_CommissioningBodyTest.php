...
{
	protected $tag = 'http://openeyes.org.uk/fhir/tag/resource/Organization/CommissioningBody';
	...
	{
		$source = file_get_contents(__DIR__ . '/files/CommissioningBody.xml');
		$this->post('Organization', $source, array('Category' => "{$this->tag}; scheme=\"http://hl7.org/fhir/tag\""));
	...
	{
		$this->get('Organization?identifier=CCG0001&_tag=' . urlencode($this->tag));
		$this->assertXPathEquals('feed', 'local-name()');
		$this->assertXPathEquals($this->client->getBaseUrl(), 'string(./atom:link[@rel="base"]/@href)');
		$this->assertXPathEquals($this->client->getBaseUrl() . '/Organization?identifier=CCG0001&_tag=' . urlencode($this->tag), 'string(./atom:link[@rel="self"]/@href)');
