 ...
 */
class CommissioningBodyService extends Service
{
	...
	/**
	 * @param CommissioningBody $resource
	 * @return int
	 */
	public function create(CommissioningBody $resource)
	{
		$model = new \CommissioningBody;
	 ...
	 * @param CommissioningBody $resource
	 */
	public function update($id, CommissioningBody $resource)
	{
	...
	public function fhirRecognised(\StdClass $fhirObject)
	{
		...
		foreach ($fhirObject->type->coding as $coding) {
			if ($coding->system == 'http://openeyes.org.uk/fhir/vs/org-type' && $coding->code == 'commissioning-body') {
				return true;
			}
		}
