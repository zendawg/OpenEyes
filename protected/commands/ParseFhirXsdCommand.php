...
class ParseFhirXsdCommand extends CConsoleCommand
{
	...
	public function run($args)
	{
		...
		$output_dir = Yii::app()->basePath . "/helpers/fhir_schema";
		...
		foreach ($xpath->query('xs:complexType') as $complexType) {
			...
			foreach ($xpath->query('.//*[@maxOccurs]', $complexType) as $item) {
				...
				foreach ($elements as $element) {
					if ($element->hasAttribute("name") && $element->hasAttribute("type")) {
						$types[$type][$element->getAttribute("name")] = array(
			...
			if (isset($types[$type])) {
				file_put_contents("$output_dir/{$type}.json", json_encode($types[$type]));
			}
		}
	}
}
