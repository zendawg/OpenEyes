...
//		}
  ...
  public function actionCreate() {
    if (isset($_POST['Patient'])
      ...
      if ($patientSearchHosNum && $patientSearchHosNum->hos_num == $this->patient->hos_num) {
        $this->render('/patient/create', array('patient_exists' => 'true'));
        return;
      }
  ...
  /**
   * Edits a patient within the 'local' DB - i.e. not in the PAS. The process
   * of chainging details is audited.
   * @param int $id the patients UID.
   */
  public function actionEdit($id) {
    if (isset($_POST['edited'])) {
      ...
      if (isset($_POST['Patient'])
              && isset($_POST['Contact'])
              && isset($_POST['Address'])) {
        $originalPatient = Patient::model()->findByPk($id);
        $this->patient = Patient::model()->findByPk($id);
        $c = $this->patient->contact;
        $a = $this->patient->address;
        $this->patient->attributes = $_POST['Patient'];
        $c->attributes = $_POST['Contact'];
        $a->attributes = $_POST['Address'];
        $this->patient->contact = $c;
        $this->patient->address = $a;
        $diffs = $this->getDiffs($originalPatient, $this->patient);
      }
    } else {
      $this->patient = Patient::model()->findByPk($id);
      $edited = false;
    }
    if (!$edited && $this->patient->validate()) {
      $this->render('/patient/edit', array('patient' => $this->patient)
      );
      return;
    }
    ...
    $patientSearchHosNum = Patient::model()->find('hos_num', array('hos_num' => $this->patient->hos_num));
    if ($patientSearchHosNum && $patientSearchHosNum->hos_num == $this->patient->hos_num) {
      $this->render('/patient/edit', array('patient_exists' => 'true'));
      return;
    }
    // NHS number must also be unique:
    $patientSearchNhsNum = Patient::model()->find('nhs_num', array('nhs_num' => $this->patient->nhs_num));
...
//
  ...
  /**
   * Gets the differences based on the edit patient form between the original
   * patient and the newly edited patient.
   * 
   * @param Patient $oldPatient the original patient to compare; non-null.
   * @param Patient $newPatient the edited (new) patient; non-null.
   * 
   * @return The differences between the two patients.
   */
  private function getDiffs($oldPatient, $newPatient) {
    $diffs = '';
    if ($oldPatient->first_name != $newPatient->first_name) {
      $diffs = $diffs . 'first name changed from ' . $oldPatient->first_name
              . ' to ' . $newPatient->first_name . '; ';
    }
    if ($oldPatient->last_name != $newPatient->last_name) {
      $diffs = $diffs . 'last name changed from ' . $oldPatient->last_name
              . ' to ' . $newPatient->last_name . '; ';
    }
    if ($oldPatient->hos_num != $newPatient->hos_num) {
      $diffs = $diffs . 'hospital number changed from ' . $oldPatient->hos_num
              . ' to ' . $newPatient->hos_num . '; ';
    }
    if ($oldPatient->nhs_num != $newPatient->nhs_num) {
      $diffs = $diffs . 'NHS number changed from ' . $oldPatient->hos_num
              . ' to ' . $newPatient->hos_num. '; ';
    }
    if ($oldPatient->dob != $newPatient->dob) {
      $diffs = $diffs . 'DOB changed from ' . $oldPatient->dob
              . ' to ' . $newPatient->dob . '; ';
    }
    if ($oldPatient->gender != $newPatient->gender) {
      $diffs = $diffs . 'gender changed from ' . $oldPatient->gender
              . ' to ' . $newPatient->gender . '; ';
    }
    
    if ($oldPatient->contact->primary_phone != $newPatient->contact->primary_phone) {
      $diffs = $diffs . 'phone changed from ' . $oldPatient->contact->primary_phone
              . ' to ' . $newPatient->contact->primary_phone . '; ';
    }
    if ($oldPatient->contact->title != $newPatient->contact->title) {
      $diffs = $diffs . 'title changed from ' . $oldPatient->contact->title
              . ' to ' . $newPatient->contact->title . '; ';
    }
    if ($oldPatient->address->address1 != $newPatient->address->address1) {
      $diffs = $diffs . 'address 1 changed from ' . $oldPatient->address->address1
              . ' to ' . $newPatient->address->address1 . '; ';
    }
    if ($oldPatient->address->address2 != $newPatient->address->address2) {
      $diffs = $diffs . 'address 2 changed from ' . $oldPatient->address->address2
              . ' to ' . $newPatient->address->address2 . '; ';
    }
    if ($oldPatient->address->postcode != $newPatient->address->postcode) {
      $diffs = $diffs . 'postcode changed from ' . $oldPatient->address->postcode
              . ' to ' . $newPatient->address->postcode . '; ';
    }
    if ($oldPatient->address->city != $newPatient->address->city) {
      $diffs = $diffs . 'city changed from ' . $oldPatient->address->city
              . ' to ' . $newPatient->address->city . '; ';
    }
    if ($oldPatient->address->email != $newPatient->address->email) {
      $diffs = $diffs . 'email changed from ' . $oldPatient->address->email
              . ' to ' . $newPatient->address->email . '; ';
    }
    return $diffs;
  }
  ...
  public function actionSummary() {
    ...
    $this->renderPartial('_summary', array('model' => $patient, 'address' => $patient->address, 'episodes' => $dataProvider));
  }
