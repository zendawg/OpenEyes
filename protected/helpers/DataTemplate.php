...
abstract class DataTemplateComponent
{
	 ...
	 */
	abstract public function match($structure);
}
...
class DataTemplateObject extends DataTemplateComponent
{
	...
	public function match($structure)
	{
		if (!is_object($structure)) return null;

		$values = array();

		foreach ($this->value as $key => $value) {
			if ($value instanceof DataTemplateComponent) {
				$new_values = $value->match(@$structure->$key);
				if (is_null($new_values)) return null;
				$values += $new_values;
			}
		}
...
class DataTemplateDirectiveMatch extends DataTemplateComponent
{
	...
	public function match($structure)
	{
		return ($structure == $this->value) ? array() : null;
	}
}
