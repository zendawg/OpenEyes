...
class FhirUtil
{
	static public $xml_error_map = array(
	 ...
	 * @return array
	 */
	static public function getSchema($type)
	{
		if (!isset(self::$schemas[$type])) {
			self::$schemas[$type] = json_decode(file_get_contents(__DIR__ . "/fhir_schema/{$type}.json"), true);
		}
	 ...
	 */
	static public function parseJson($text)
	{
	 ...
	 * @return StdClass|null
	 */
	static public function parseXml($text)
	{
		$doc = new DOMDocument;
		...
		if ($root->tagName == 'feed') {
			$obj = self::parseXmlBundle($doc, $root);
			$obj->resourceType = "Bundle";
		} else {
			$obj = self::parseXmlRecursive($doc, $root, $root->tagName, self::getSchema($root->tagName));
	...
	static private function parseXmlRecursive(DOMDocument $doc, DOMElement $element, $type, array $schema)
	{
		$obj = new StdClass;
		...
		foreach ($element->childNodes as $child) {
			...
			$child_schema = self::getSchema($child_type);

			if ($type == 'Resource.Inline') {
				$obj = self::parseXmlRecursive($doc, $child, $child_type, $child_schema);
			...
			if ($is_primitive) {  // of course primitive types require far more complex processing
				...
				if ($child->hasAttribute("id") || $child->hasChildNodes()) {
					$extra = self::parseXmlRecursive($doc, $child, $child_type, $child_schema);
				} else {
					$extra = null;
				}
			} else {
				$value = self::parseXmlRecursive($doc, $child, $child_type, $child_schema);
			}
	...
	static public function parseXmlBundle(DOMDocument $doc, DOMElement $element)
	{
		$obj = new StdClass;
		...
		foreach ($element->childNodes as $child) {
			...
			switch ($child->tagName) {
				...
				case 'content':
				case 'summary':
					$obj->{$child->tagName} = self::parseXmlBundle($doc, $child);
					break;
				case 'entry':
					$obj->entry[] = self::parseXmlBundle($doc, $child);
					break;
				case 'Signature':
					$obj->signature = $doc->saveXml($child);
					break;
				default:
					$obj = self::parseXmlRecursive($doc, $child, $child->tagName, self::getSchema($child->tagName));
	 ...
	 * @return string
	 */
	static public function renderJson(StdClass $resource)
	{
	 ...
	 * @return string
	 */
	static public function renderXml(StdClass $resource)
	{
		...
		if ($resource->resourceType == "Bundle") {  // stupid special case
			...
			self::renderXmlBundle($resource, $doc, $root);
		} else {
			self::renderXmlRecursive($resource, $doc, $doc);
		}
	...
	static private function renderXmlRecursive(StdClass $data, DOMDocument $doc, DOMNode $parent)
	{
		...
		foreach ($data as $name => $value) {
			...
			$extras = is_array($extra) ? $extra : array($extra);
			...
			for ($i = 0; $i < $n; $i++) {
				...
				if (is_object($value)) {
					self::renderXmlRecursive($value, $doc, $el);
				...
				if ($extra) self::renderXmlRecursive($extra, $doc, $el);
			}
		}
	}

	static private function renderXmlBundle(StdClass $data, DOMDocument $doc, DOMNode $parent)
	{
		foreach ($data as $name => $value) {
			...
			foreach ($values as $value) {
				...
				switch ($name) {
					...
					case 'updated':
						...
						break;
					case 'entry': // recur
						self::renderXmlBundle($value, $doc, $el);
						break;
